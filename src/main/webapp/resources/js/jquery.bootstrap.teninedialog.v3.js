...
(function($) {
	$.fn.teninedialog = function(options) {
		var defaults = {
			...
			content: '',
			url: '', //远程URL
			showIframe :false,//是否以iframe的形式出现，只前当URL不为空的时候才有效
		...
		return this.each(function() {
			...
			var tmpHtml = '<div class="modal fade" id="{ID}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel_{ID}" aria-hidden="true"><div class="modal-dialog" style="'+ options.css +'"><div class="modal-content" style="height:100%;"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title" id="myModalLabel_{ID}">{title}</h4></div><div class="modal-body" id="body_{ID}" style="overflow-y: auto;height:100%;max-height: 900px;background-color:white;padding:10px 0px 10px 10px;">{body}</div><div class="modal-footer"><img id="ajax-loader" style="display:none" height="32" width="32" src="' + loadimg + '" />{button}</div></div></div></div>';
			...
			if(options.otherButtons != null && options.otherButtons != ""){
			...
			modalObj.on('show.bs.modal', function() {
				...
				if (options.url != '') {
					if(options.showIframe){
						$('#ajax-loader').hide();
						var iframeNode = "<iframe style='border:0;height:98%;width:100%;' src='"+options.url+"'></iframe>";
						$('#body_' + modalID).html(iframeNode);
					}else{
						$('#ajax-loader').show();
						$.ajax({
							type: "get",
